...
int main() {
    ...
    FILE *fp = fopen("Av_Length_random", "w");
    ...
    for (size_t t = 0; t < T; ++t) {
        ...
        int randx = random() % L;
        int randy = random() % L;
        M[randx][randy]++; // increment random cell

        if (av_length != 0) {
            fprintf(fp, "%d \t", av_length);
            av_length = 0;
        }

        if (M[randx][randy] > 3)overloaded.insert(randx * L + randy);
