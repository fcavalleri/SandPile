...
struct next_t {};
template<>
struct next_t<uint64_t> { typedef uint32_t type; };
template<>
struct next_t<uint32_t> { typedef uint16_t type; };
template<>
struct next_t<uint16_t> { typedef uint8_t type; };

template<typename Max_t, Max_t upto_n, typename Best_t=Max_t, typename Try_t=uint64_t, bool try_is_better = (
    sizeof(Try_t) <= sizeof(Best_t) && upto_n == Max_t(Try_t(upto_n)))>
struct tight_int {
...
class udcs {
  // unsigned_dat_compact_set
public:
  udcs(T size = 0) : ssize(size) {
