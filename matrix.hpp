...
public:
  ...
  //iteration
  std::tuple<size_t> index_of(const T* x) const {return {x - base_t::data()};}
...
class matrix<T, N1, N2, Ns...> : public std::array<matrix<T, N2, Ns...>, N1> {
  typedef std::array<matrix<T, N2, Ns...>, N1> array_t;
  template<typename U, size_t M, size_t ...Ms> friend
  class matrix;
public:
...
template<typename T, size_t N1, size_t N2, size_t... Ns>
auto matrix<T, N1, N2, Ns...>::index_of(const T* x) const {
  size_t idx_1 = (x - flat().data()) / ((N2) * ... * Ns);
  return std::tuple_cat(std::make_tuple(idx_1),array_t::at(idx_1).index_of(x));
}
...
template<typename T, size_t N1, size_t N2, size_t... Ns>
template<size_t... Ws>
constexpr const matrix<T, Ws...> &matrix<T, N1, N2, Ns...>::reshape() const noexcept {
  static_assert(matrix<T, N1, N2, Ns...>::_items_size == matrix<T, Ws...>::_items_size);
  static_assert(sizeof(matrix<T, N1, N2, Ns...>) == sizeof(matrix<T, Ws...>));
  return *reinterpret_cast<const matrix<T, Ws...> *>(this);
}
